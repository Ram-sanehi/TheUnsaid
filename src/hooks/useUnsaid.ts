...
export function useUnsaid(chapterId: string) {
  ...
  const isMarked = useCallback((paragraphIndex: number) => {
    return load().some(m => m.chapterId === chapterId && m.paragraphIndex === paragraphIndex);
  }, [chapterId]);

  const toggleUnsaid = useCallback((paragraphIndex: number, text: string) => {
    const all = load();
    const existing = all.find(m => m.chapterId === chapterId && m.paragraphIndex === paragraphIndex);
    if (existing) {
      ...
      const mark: UnsaidMark = {
        id: crypto.randomUUID(), chapterId, paragraphIndex, text, createdAt: Date.now(),
      };
      all.push(mark);
      save(all);
